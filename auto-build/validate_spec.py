...
Usage:
    ...
    python auto-build/validate_spec.py --spec-dir auto-build/specs/001-feature/ --checkpoint context
...
class SpecValidator:
    ...
    def validate_prereqs(self) -> ValidationResult:
        ...
        if not project_index.exists():
            # Check if it exists at auto-build level
            auto_build_index = self.spec_dir.parent.parent / "project_index.json"
            if auto_build_index.exists():
                warnings.append("project_index.json exists at auto-build/ but not in spec folder")
                fixes.append(f"Copy: cp {auto_build_index} {project_index}")
            else:
                errors.append("project_index.json not found")
                fixes.append("Run: python auto-build/analyzer.py --output auto-build/project_index.json")
    ...
    def validate_context(self) -> ValidationResult:
        ...
        if not context_file.exists():
            ...
            fixes.append("Run: python auto-build/context.py --task '[task]' --services '[services]' --output context.json")
    ...
    def validate_implementation_plan(self) -> ValidationResult:
        ...
        if not plan_file.exists():
            ...
            fixes.append(f"Run: python auto-build/planner.py --spec-dir {self.spec_dir}")
        ...
        try:
            ...
            fixes.append("Regenerate with: python auto-build/planner.py --spec-dir " + str(self.spec_dir))
            return ValidationResult(False, "plan", errors, warnings, fixes)
