...
/**
 * Get effective width for a column (accounts for collapsed state)
 */
export function getEffectiveColumnWidth(column: TaskStatusColumn): number {
  const prefs = useKanbanSettingsStore.getState().getColumnPreferences(column);
  return prefs.isCollapsed ? COLLAPSED_COLUMN_WIDTH : prefs.width;
}

/**
 * Check if any column is currently collapsed
 */
export function hasAnyCollapsedColumn(): boolean {
  const state = useKanbanSettingsStore.getState();
  if (!state.columnPreferences) return false;

  return TASK_STATUS_COLUMNS.some(
    (column) => state.columnPreferences![column].isCollapsed
  );
}

/**
 * Get count of collapsed columns
 */
export function getCollapsedColumnCount(): number {
  const state = useKanbanSettingsStore.getState();
  if (!state.columnPreferences) return 0;
 ...
 */
export function expandAllColumns(): void {
  const store = useKanbanSettingsStore.getState();
  if (!store.columnPreferences) return;

  for (const column of TASK_STATUS_COLUMNS) {
    if (store.columnPreferences[column].isCollapsed) {
      store.setColumnCollapsed(column, false);
    }
  }
}
